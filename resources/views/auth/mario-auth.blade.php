...
<body>
    <div class="container">
        <div class="auth-box" id="authBox">
            <div class="auth-title" id="formTitle">{{ $isRegister ? 'Criar Conta' : 'Login' }}</div>
            <div class="auth-desc" id="formDesc">{{ $isRegister ? 'Crie sua conta para começar a usar o sistema' : 'Acesse o sistema de emissão de etiquetas' }}</div>
            <form method="POST" action="{{ $isRegister ? route('register') : route('login') }}" id="authForm">
                @csrf
                <div id="registerFields" style="display: {{ $isRegister ? 'block' : 'none' }};">
                    <div class="input-group">
                        <label for="name" class="input-label">Nome</label>
                        <input id="name" class="input-field" type="text" name="name" value="{{ old('name') }}" {{ $isRegister ? 'required autofocus' : '' }} />
                ...
                <div id="confirmField" class="input-group" style="display: {{ $isRegister ? 'block' : 'none' }};">
                    <label for="password_confirmation" class="input-label">Confirmar Senha</label>
                    <input id="password_confirmation" class="input-field" type="password" name="password_confirmation" {{ $isRegister ? 'required' : '' }} />
                    <x-input-error :messages="$errors->get('password_confirmation')" class="error" />
                </div>
                <button type="submit" class="auth-btn" id="submitBtn">{{ $isRegister ? 'Cadastrar' : 'Entrar' }}</button>
            </form>
            <span class="toggle-link" id="toggleAuth">{{ $isRegister ? 'Já tem uma conta? Faça login' : 'Não tem uma conta? Cadastre-se' }}</span>
        </div>
    </div>
    <script>
